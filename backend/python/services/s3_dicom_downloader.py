...
class S3DicomDownloader(DicomDownloader):
    def __init__(self):
    ...
    def download(self, source_path: str, dest_path: str) -> None:
        """
        Downloads a DICOM file from S3 to a local destination path.
        
        Parameters:
        - source_path: The S3 object path (e.g., 'BUCKET_NAME/folder/subfolder/dicomfile.dcm')
        - dest_path: The local file path to save the DICOM file (e.g., '/local/path/dicomfile.dcm')
        """
        self.download_dicom_file(source_path, dest_path)
